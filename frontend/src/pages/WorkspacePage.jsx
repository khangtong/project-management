    ...
    <div className="relative">
      <div className="relative flex items-center">
        ...
        <div className="absolute right-2.5 pointer-events-none flex items-center">
          ...
          {selected && (
            <button type="button" onClick={handleClear} className="pointer-events-auto text-gray-medium hover:text-charcoal">
      ...
      {open && query.length >= 2 && (
        <div ref={dropdownRef} className="absolute z-20 mt-1 w-full bg-white border border-cream-border rounded-xl shadow-lg overflow-hidden">
          {results.length > 0 ? (
            ...
            <>
              <div className="px-3 py-3 text-sm text-gray-medium text-center">No users found for <span className="font-medium text-charcoal">{query}</span></div>
              <EmailFallback query={query} onSelect={handleSelect} />
            </>
          ) : null}
        </div>
        ...
        <div className="p-6">
          ...
          <form onSubmit={(e) => { e.preventDefault(); mutation.mutate({ name, description }); }} className="space-y-4">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required autoFocus className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal focus:ring-2 focus:ring-ocean focus:border-ocean" />
            </div>
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="What is this workspace for?" className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal resize-none focus:ring-2 focus:ring-ocean focus:border-ocean" />
            </div>
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-cream-border overflow-hidden">
        ...
        <div className="p-6">
          ...
          <form onSubmit={(e) => { e.preventDefault(); mutation.mutate({ name, description, color, status }); }} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-charcoal mb-1">Description <span className="text-gray-medium font-normal">(optional)</span></label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="What is this project about?" className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal resize-none focus:ring-2 focus:ring-ocean focus:border-ocean" />
            </div>
            <div>
              <label className="block text-sm font-medium text-charcoal mb-2">Color</label>
              <div className="flex gap-2 flex-wrap">
                {PROJECT_COLORS.map((c) => (
                  <button key={c} type="button" onClick={() => setColor(c)} className={`w-7 h-7 rounded-full transition-all ${color === c ? "scale-125 ring-2 ring-offset-2 ring-charcoal/20" : "hover:scale-110"}`} style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-charcoal mb-1">Status</label>
              <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal text-sm focus:ring-2 focus:ring-ocean focus:border-ocean">
  ...
  return (
    <div className="mb-6 rounded-2xl bg-white shadow-sm border border-cream-border overflow-hidden">
      ...
      <div className="p-6">
        ...
        <form
          onSubmit={(e) => { e.preventDefault(); if (!name.trim()) return; mutation.mutate({ name, description, color, status }); }}
          className="space-y-4"
        >
          {/* Name — always visible */}
          <div>
            ...
            <div className="space-y-4 pt-1">
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal resize-none focus:ring-2 focus:ring-ocean focus:border-ocean text-sm"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {PROJECT_COLORS.map((c) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setColor(c)}
                        className={`w-6 h-6 rounded-full transition-all ${color === c ? "scale-125 ring-2 ring-offset-1 ring-charcoal/20" : "hover:scale-110"}`}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal text-sm focus:ring-2 focus:ring-ocean focus:border-ocean"
                  >
                    <option value="active">Active</option>
                    <option value="on_hold">On Hold</option>
                    <option value="archived">Archived</option>
      ...
      <div className="max-w-5xl mx-auto p-8">
        ...
        <div className="mb-8 rounded-2xl bg-white shadow-sm border border-cream-border overflow-hidden">
          ...
          <div className="p-6">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-medium hover:text-ocean transition-colors mb-4"
            >
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                {workspace?.description && (
                  <p className="text-sm text-gray-medium mt-1.5 leading-relaxed max-w-lg">{workspace.description}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 mt-0.5">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 text-sm font-medium rounded-lg transition-colors ${showInvite ? "bg-ocean text-white" : "bg-ocean text-white hover:bg-ocean/90"}`}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className="p-2 rounded-lg text-gray-medium hover:text-ocean hover:bg-ocean/10 transition-colors"
                ...
                <button
                  ...
                  className="p-2 rounded-lg text-gray-medium hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-40"
        ...
        {showInvite && (
          <div className="mb-6 p-6 rounded-2xl bg-white shadow-sm border border-cream-border">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base font-semibold text-charcoal">Invite a Member</h3>
                <p className="text-xs text-gray-medium mt-0.5">Existing users are added instantly. Others receive an email link.</p>
              </div>
              <button onClick={() => { setShowInvite(false); setSelectedUser(null); }} className="p-1.5 rounded-lg text-gray-medium hover:bg-gray-100 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <form
              onSubmit={(e) => { e.preventDefault(); if (!selectedUser?.email) return; inviteMutation.mutate({ email: selectedUser.email, role: inviteRole }); }}
              className="space-y-4"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1">Search user</label>
                  <UserSearchCombobox workspaceId={id} onSelect={setSelectedUser} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1">Role</label>
                  <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-cream-border text-charcoal text-sm focus:ring-2 focus:ring-ocean">
              ...
              <div className="flex gap-3">
                <button type="submit" disabled={!selectedUser || inviteMutation.isPending} className="px-5 py-2 rounded-lg font-medium text-sm text-white bg-ocean hover:bg-ocean/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                  {inviteMutation.isPending ? "Adding…" : selectedUser?._emailOnly ? "Send Invite" : "Add Member"}
                </button>
                <button type="button" onClick={() => { setShowInvite(false); setSelectedUser(null); }} className="px-5 py-2 rounded-lg border border-cream-border text-sm text-gray-medium hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
              </div>
            </form>
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          {!showCreateProject && (
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium rounded-lg text-white bg-ocean hover:bg-ocean/90 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showCreateProject && (
          <CreateProjectForm
            ...
            onSuccess={(project) => {
              ...
              if (project?.id) navigate(`/projects/${project.id}/board`);
            }}
        ...
        {projects.length === 0 && !showCreateProject ? (
          <div className="text-center py-16 rounded-2xl border-2 border-dashed border-cream-border">
            ...
            <p className="text-sm text-gray-medium mt-1">Create your first project to start managing tasks</p>
          </div>
        ) : (
                ...
                <div className="flex-1 p-5">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => setEditingProject(project)} className="p-1.5 rounded-lg text-gray-medium hover:text-ocean hover:bg-ocean/10 transition-colors" title="Edit">
                        <EditIcon />
                      </button>
                      <button onClick={() => handleDeleteProject(project)} disabled={deleteProjectMutation.isPending} className="p-1.5 rounded-lg text-gray-medium hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-30" title="Delete">
                        <TrashIcon />
                      </button>
                    </div>
                  </div>
                  ...
                  {project.description && (
                    <p className="text-sm text-gray-medium line-clamp-2 mb-3">{project.description}</p>
                  )}
