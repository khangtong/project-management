...
export default function KanbanColumn({ column, tasks = [], onTaskClick }) {
    ...
    return (
        <div
            ...
            style={{ backgroundColor: '#FAF0E4', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: `4px solid ${accentColor}` }}
        >
                ...
                {isEditing ? (
                    <form onSubmit={handleRename} className="flex-1 flex gap-1">
                        <input
                            ...
                            autoFocus
                        />
                        <button type="submit" className="text-xs px-2 py-1 rounded font-medium text-white bg-ocean">
                            Save
                        </button>
                    </form>
                ) : (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded-full text-white" style={{ backgroundColor: accentColor }}>
                            {tasks.length}
                        </span>
                    </div>
                )}
            ...
            {isAdding ? (
                ...
                <button
                    ...
                    className="mx-3 mb-3 w-full py-2 text-sm flex items-center gap-1 rounded-md hover:bg-white/50 transition-colors text-mint"
                >
